...
def handler(event, context):
    ...
    try:
        ...
        # Step 1: Fetch all submissions for the contract using the GSI
        ...
        # Step 3: Invoke Bedrock to select a winner from the submissions
...
def select_winner(contract: dict, submissions: list) -> dict:
    ...
    for sub in submissions:
        submissions_text += (
            f"<submission>\n"
            f"  <id>{sub['submission_id']}</id>\n"
            f"  <content>{sub['submission_data']}</content>\n"
            f"</submission>\n"
        )
